...
which nets > /dev/null || ( echo "can't locate utility 'nets'" ; exit 1 )
which vm > /dev/null || ( echo "can't locate utility 'vm'" ; exit 1 )
...
function get_script {
    if [[ -f $1 ]] ; then eval ${2}="$1" ; ln -srf "$1" ${scriptd}/${2}
    elif [[ -f ${scriptd}/${1} ]] ; then eval ${2}="${scriptd}/${1}" ; ln -srf "${scriptd}/$1" ${scriptd}/${2}
    # elif [[ -f ${scriptd}/${2} ]] ; then link=$(realpath "${scriptd}/${2}") ; eval ${2}="${link}"
    elif [[ -f ${scriptd}/${2} ]] ; then eval ${2}=$(realpath "${scriptd}/${2}")
    else echo "$1 is not found and no saved default, nothing to do"; exit 1 ; fi
    echo "using ${!2} as ${2}"    
}
