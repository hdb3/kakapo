...
int main(int argc, char *argv[]) {
  ...
  int reuse = 1;
  if (setsockopt(serversock, SOL_SOCKET, SO_REUSEADDR, (const char *)&reuse,
                 sizeof(reuse)) < 0)
    die("Failed to set server socket option SO_REUSEADDR");

  if (setsockopt(serversock, SOL_SOCKET, SO_REUSEPORT, (const char *)&reuse,
                 sizeof(reuse)) < 0)
  ...
  while (1) {
    ...
    if ((peersock =
             accept(serversock, (struct sockaddr *)&peeraddr, &addrsize)) < 0) {
