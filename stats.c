...
char *displaylogrecord(slp_t slp) {
  ...
  int tmp = asprintf(
      &s,
      "f update msg cnt  %6ld (%6ld) NLRI cnt  %6ld (%6ld) withdrawn cnt  %6ld "
      "(%6ld)",
      // //int tmp = asprintf(&s,"elapsed time : %f update msg cnt  %6ld (%6ld)
      // NLRI cnt  %6ld (%6ld) withdrawn cnt  %6ld (%6ld), last burst %f" ,
      //  (now - slp->cumulative.ts ) / 1e6,
      slp->current.updates, slp->cumulative.updates, slp->current.nlri,
      slp->cumulative.nlri, slp->current.withdrawn, slp->cumulative.withdrawn
...
char *displaysessionlog(slp_t slp) {
  ...
  pthread_mutex_lock(&slp->mutex);
  if (0 != slp->firstts.tv_sec)
    snprintf(bm, 255, "current burst %f",
             timespec_to_double(timespec_sub(slp->lastts, slp->firstts)));
  int tmp =
      asprintf(&s,
               "elapsed time : %f update msg rate %6ld (%6ld) NLRI rate %6ld "
               "(%6ld) withdrawn rate %6ld (%6ld) %s",
               slp->cumulative.ts / 1e6, slp->current.updates,
               slp->cumulative.updates, slp->current.nlri, slp->cumulative.nlri,
               slp->current.withdrawn, slp->cumulative.withdrawn, bm);
  pthread_mutex_unlock(&slp->mutex);
  return s;
};
...
void getsessionlog(slp_t slp, slp_t slog) {
  ...
  slog->cumulative.withdrawn =
      slp->cumulative.withdrawn * _1e6 / deltaCumulative;
    ...
    if (0 == slp->closed) {
      ...
      if (SHOWRATE) {
        ...
        fprintf(stderr, "%s: counters: %s\e[K\n", slp->tids,
                displaylogrecord(slp));
        fprintf(stderr, "%s: rate:     %s\e[K\n", slp->tids,
                displaysessionlog(&tmp));
        ///} else if (b) {
        /// getsessionlog(slp,&tmp);
        /// fprintf(stderr, "%s: counters: %s\n",slp->tids,displaylogrecord
        /// (slp)); fprintf(stderr, "%s: rate: %s\n",slp->tids,displaysessionlog
        /// (&tmp));
      };
    };
    slp = slp->next;
