...
get_psu_status() {
  if
    [[!-f "/sys/class/power_supply/AC/online"]]; then
    echo "AC"
  else
...
#must call set_docker_vars before the later ENV VAR settings, because set_docker_vars sets some inputs to them....
set_docker_vars

KVARS="NOPACK TIMEOUT RATEBLOCKSIZE MAXBLOCKINGFACTOR REPEAT IDLETHR SEEDPREFIX CANARYSEED SEEDPREFIXLEN GROUPSIZE RATEWINDOW TABLESIZE MAXBURSTCOUNT RATECOUNT RATETIMELIMIT PEERMAXRETRIES REPEATDELAY TCPPORT SHOWRATE HOLDTIME LOGFILE LOGPATH LOGTEXT SENDFILENAME MODE"
...
do
  PEERS = "$PEERS 172.18.0.13,172.18.0.${n},64504" done
...
#set_docker_vars
  ...
  for envvar in $KVARS;
  do
    ENVSTR = "$ENVSTR --env $envvar" if[[!-z "${!envvar}"]];
  then
      echo "$envvar is ${!envvar}" export ${envvar} fi
          done
              CURDIR = "$(realpath $PWD)" touch $CURDIR / kakapo.json
                                                              MAP_KAKAPO_JSON = "--volume $CURDIR/kakapo.json:/kakapo.json" KAKAPO_COMMAND = "$DOCKER_RUN_BASE -it $ENVSTR $MAP_KAKAPO_JSON --name kakapo kakapo 172.18.0.13,172.18.0.19,64505 $PEERS"
#echo "kakapo command is $KAKAPO_COMMAND"
      eval "$KAKAPO_COMMAND"
  }

docker_clean(){
  docker kill $CONTAINERS & > / dev / null || : docker rm $CONTAINERS & > / dev / null || :
}

docker_stop_wait() {
  docker kill $1 & > / dev / null || : docker rm $1 & > / dev / null || : while docker container inspect-- format '{{.ID}}' $1 & > / dev / null;
  do
    echo - n '.' sleep 1.0 done
               echo "killed $1"
}
...
set_command() {
  ...
  ;
      ...
      if[[-z "${COMMAND}"]];
  then
      echo "unknown daemon: $1" exit 1 fi
...
then
            $SCRIPT_DIR /
        add_loopbacks.sh del lo $((N_PEERS + 1)) & >
    / dev / null $SCRIPT_DIR / add_loopbacks.sh add virbr1 $((N_PEERS + 1)) & > / dev / null else $SCRIPT_DIR / add_loopbacks.sh del virbr1 $((N_PEERS + 1)) & > / dev / null $SCRIPT_DIR / add_loopbacks.sh add lo $((N_PEERS + 1)) & > / dev / null fi

                                                                                                                                                                                                                                             docker_clean CMND = $(set_command $1)
                                                                                                                                                                                                                                                                     bash
                                                                                                                                                                                                                                                                 - c "${CMND}" run_kakapo
                                                                                                                                                                                                                                                                 docker_stop_wait $1
                                                                                                                                                                                                                                                                 docker_clean
